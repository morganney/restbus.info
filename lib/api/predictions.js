...
predictions.location = function(req, res) {// /locations/:lat,:lon/predictions
  var p = req.params, latlon = p.latlon.split(','), lat = latlon[0], lon = latlon[1],
      options = {
        hostname: 'www.nextbus.com',
        path: ['/webkit/predsByLoc.jsp?lat=', lat, '&lon=', lon, '&maxDis=23000&maxNumStops=320'].join(''),
  ...
  http.get(options, function(nbres) {
    ...
    nbres.on('data', function(d) { if(d) html += d; });

    nbres.on('end', function() {
      var $ = cheerio.load(html.trim(), { normalizeWhitespace: false }), json = [];

      $('div.plainText').each(function(idiv, div) {
        var p = {};

        p.agencyTitle = $('span.agencyName', this).text();
        p.predstrings = [];
        $('span.preds', this).each(function(ipredstr, predstr) {
          p.predstrings.push($(predstr).text());
        });
        json.push(p);
      });
